...
Trezor T also supports pre-signed inputs for multi-party signing. If an input has an
...
Trezor will ask once again about every input, begin emitting a serialization of the
transaction and signing legacy (non-segwit) inputs. For every legacy input, it is
necessary to stream the full set of inputs and outputs again, so that Trezor can build
the serialization which is being signed. For segwit inputs, this is not necessary
